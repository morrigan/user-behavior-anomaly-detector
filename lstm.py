...
def predict(model, actions):
	...
	#actions.reshape(-1, max_vector_length)

	x, y = helpers.get_real_predictions(actions)

	#actions = numpy.reshape(actions, (actions.shape[0], 1, max_vector_length))

	predicted = model.predict(x, batch_size=1, verbose=0)
	#rmse = numpy.sqrt(((predicted - y) ** 2).mean(axis=0))
	score = math.sqrt(mean_squared_error(y, predicted))
	print score
	return predicted
